...
def plot_data(**kwargs):
    ...
    fig, ax = plt.subplots(figsize=plt_args['figsize'])
 ...
 #   ax2.plot(
  #      dataDict[4]['x'],
   ...
   #     c='orange')
    ...
    ax.plot(
        dataDict[5]['x'],
        abs(dataDict[5]['eta_NL']),
        lw=plt_args['lw'] + 2.,
        c='g')

#    nB = lambda x: 1. / (np.exp(x) - 1.)
#    argnbL = float(parDict[2]['wL']) / float(parDict[2]['TOscL'])
#    argnbR = float(parDict[2]['wR']) / float(parDict[2]['TOscR'])
#    axins.plot(
#        dataDict[2]['x'],
#        dataDict[2]['nOscR'] / nB(argnbR),
#        ls='-.',
#        c='r',
#        lw=plt_args['lw'],
#        label='nR')
...
#    #    ax2.yaxis.set_label_coords(0.0, 5., transform=trans)
    ...
    ax.set_xlim(0.706, 0.712)
    ax.set_ylim(0, 1.)
    ax.set_xticks([0.706, 0.709, 0.712])
    ax.set_yticks([0, 0.25, 0.5, 0.75, 1])
    ax.set_yticklabels([0, '', '', '', 1])

    ax.set_xlabel(
        'eps', fontsize=plt_args['fontsize'], labelpad=plt_args['labelpad'])
    ax.set_ylabel('etaNL', fontsize=plt_args['fontsize'], labelpad=0)
...
#        fontsize=plt_args['fontsize'])
    ...
    plt.subplots_adjust(
        left=0.09, right=0.95, top=0.955, bottom=0.15, wspace=0.2, hspace=0.35)
